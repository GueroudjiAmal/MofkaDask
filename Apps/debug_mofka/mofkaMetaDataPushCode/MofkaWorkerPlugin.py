...
class MofkaWorkerPlugin(WorkerPlugin):
    ...
    def teardown(self, worker):
        ...
        teardown = {"time" : "0"}
        try:
            f = self.producer.push({"action": "remove_worker", "time" : str(time.time())}, str(teardown).encode("utf-8"))
            f.wait()
    ...
    def transition(
        ...
        **kwargs: Any,
        ...
        if "custom-placeholder-key" == str(key):
            ...
            print("Worker flush")
            self.producer.flush()
            print("Worker completed flush")
        
        # transition_data = str({ "key" : str(key)}).encode("utf-8")
        ...
        try:
            f = self.producer.push({"action": "worker_transition", "key"            : str(key),
                            "start"          : str(start),
                            "finish"         : str(finish),
                            "called_from"    : str(self.worker.name),
                            "time"           : str(time.time())}, transition_data)
            # f.wait()
        except Exception as Argument:
            logging.exception("Exception while calling transition method when sending", transition_data)
        ...
        if l2 > l:
            ...
            for d in data:
                try:
                    ...
                    print("Type D", type(d))
                    print("D", d)
                    dd = str({"start":"0"}).encode("utf-8")
                    # print("DD: ", dd)
                    
                    base = {"action": "worker_transfer"}
                    combo = base | d
                    print("Combo: ", combo)
                    f = self.producer.push(combo, dd)
                    # f.wait()
                except Exception as Argument:
