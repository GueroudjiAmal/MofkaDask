...
for R in 1  #{1..$RUNS}
do
    NNODES=$(($NWORKER /2 + 4)) #2 workers per node, one node for client and one for scheduler one for Mofka consumer and one for mofka server
    mkdir -p MOFKA
    DATE=$(date +"%Y-%m-%d_%T")
    WORKSPACE=$DIR/MOFKA/D${DATE}_W${NWORKER}/
    mkdir  -p $WORKSPACE
    cd $WORKSPACE
    cp -r  $DIR/*.py  $DIR/scripts/* $DIR/*.json custom  .
